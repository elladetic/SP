...
summary(model1) #dob i iznos zavisni, ovaj model je sve u svemu dobar 

model2 <- lm(auto$iznos ~ auto$dob + auto$dob * factor(auto$prijava)) #dodavanje interekacije
...
## f dio
```{r}
#uvođenje varijabla u model obrnutim redoslijedom i ponovo dio d
...
model2 <- lm(auto$iznos ~ auto$spol + auto$zupanija + auto$dob * factor(auto$prijava) + auto$br.uzdr.cl + auto$udio.stete + auto$prijava) #dodavanje interekacije
summary(model2) 
